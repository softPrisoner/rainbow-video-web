<template>
  <header>
    ...
    <!--发布朋友圈动态-->
    <div class="cameraPart" v-if="cameraPart" @click="changeState()">
      <svg class="camera-pic">
        <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#cameratop"></use>
      </svg>
      <input class="coverfile" id="input_file" style="opacity: 0" type="file" name="cover" accept="image/*"
             capture="camera"
             multiple="multiple"/>
    </div>
    <!-- 下拉框 -->
    <section class="selectpart" v-show="addthing">
      ...
      <div class="selectlist">
        <ul>
          <router-link to='/addressbook' tag="li" class="selectpart_li">
          ...
          <section @click="callAndroidQRCode" class="selectsvg">
            <svg fill="#fff">
...
<script>
  ...
  export default {
    ...
    props: ['logoPart', 'crossover', 'searchPart', 'add', 'person', "search", "clickrefresh", 'cameraPart'],
    created() {
    ...
    changeState() {
      this.$store.state.sheetVisible = true;
      //调用手机照相和相册功能
      callMedia.callAndroidCamera()
    }
  }
</script>
