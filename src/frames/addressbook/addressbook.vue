      ...
      <div class="contacts_bottom" ref="addlistfather">
            ...
            <ul>
              <router-link to="/addressbook/details" tag="li" v-for="(item, index) in value"
                ...
                <div class="personlist_name ellipsis">
                  <!--//判断是否与备注名如果有备注名显示备注名-->
                  {{item.remarks ? item.remarks : item.pet_name}}
                </div>
              </router-link>
            </ul>
          </li>
        ...
        <section class="guide_wipe">
          <section class="list_guide">
            <dl>
              <dd v-for="(value, index) in sortlist" :key="index" @touchstart="startThing(value)" @touchend="endThing">
                {{value}}
              </dd>
            </dl>
            <p>#</p>
