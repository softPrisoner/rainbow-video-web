<template>
  <section>
      ...
      <section slot="searchpart">
        <section class="searchpart">
          ...
          <div class="searchpart_input">
            <input type="text" :placeholder="reminder" v-model="searchCondition"/>
            <div>
            </div>
          </div>
        </section>
    ...
    <section class="search" v-show="searchCondition" @click="searchItem">
      ...
      <section class="selectext">
        搜索:<font color="#006400">{{searchCondition}}</font>
      </section>
    </section>
    <section class="search" v-show="!searchCondition">
      <ul class="clear">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchItem() {
        this.$axios.post("http://192.168.43.52:8087/v1/user/search",
          {
          ...
          if (response.status == 200) {
            ...
            if (result.data != null) {
              ...
              this.SAVE_MESSAGE(this.searchData);
              // alert("sasdisdiushsu" + JSON.stringify(result))
...
<style lang="scss" scoped>
  ...
  .search {
    ...
    .selectsvg {
      ...
      float: left;
      @include widthHeight(1.24rem, 1.133333rem);
